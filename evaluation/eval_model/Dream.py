...
class Dream(LM):
    ...
    def _generate_batch(self, prompts: List[str]) -> List[str]:
        ...
        return responses,generation_ids.token_per_inference.cpu()
    ...
    def generate_until(self, requests: List[Instance], disable_tqdm: bool = False):
        ...
        for batch_idx in range(0, len(requests), self.batch_size):
            ...
            tokens_per_infer_list.append(token_per_inference)
            
        local_tokens_per_infer = np.mean(tokens_per_infer_list)
        # 将 numpy.float32 转换为 torch.Tensor
        local_tokens_per_infer = torch.tensor(local_tokens_per_infer, dtype=torch.float32).to(self.device)
        # 使用 dist.all_reduce 进行全局归约
        dist.all_reduce(local_tokens_per_infer)
        local_tokens_per_infer /= dist.get_world_size()
        print(f"tokens per infer: {local_tokens_per_infer.item()}")
        return res
