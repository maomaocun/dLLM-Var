...
class LLaDA(TemplateLM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 检测是否有可用的GPU，如果有则将device设置为'cuda'
    ...
    def generate_until(self, requests: List[Instance]) -> List[str]:
        ...
        if isinstance(gen_kwargs["until"], str):
            gen_kwargs["until"] = gen_kwargs["until"].split(" ")
        tokens_per_generate_list = []

        print(f"self.enable_editing{self.enable_editing}, self.mask_id{self.mask_id}, self.remasking{self.remasking}")
        ...
        for batch in ds.iter(self.batch_size):
            ...
            result = self.model.generate(
                ...
                max_gen_length=gen_kwargs.get("max_gen_length"),
                block_length=gen_kwargs.get("block_length"),
                threshold=gen_kwargs.get("confidence_threshold"),
                eos_token_id=126348 
            )
            torch.cuda.empty_cache()
            tokens_per_generate_list.append(result.shape[0]*result.shape[1])
            cont_toks_list = self.tokenizer.batch_decode(result, skip_special_tokens=False)
            for s in cont_toks_list:
                if not self.escape_until:
                    for term in [self.tokenizer.eos_token]+gen_kwargs.get("until")+["<|eot_id|>"]:
                        if len(term) > 0:
                            s = s.split(term)[0]
                res.append(s)
                bar.update(1)
        ...
        elapsed_time_ms = start_event.elapsed_time(end_event)  # 单位：毫秒
        elapsed_time_s = elapsed_time_ms / 1000.0  # 转换为秒

        bar.close()
        total_token = np.sum(tokens_per_generate_list)
        total_token = torch.tensor(total_token, dtype=torch.float32).to(self.device)
        elapsed_time_s = torch.tensor(elapsed_time_s, dtype=torch.float32).to(self.device)
        dist.all_reduce(total_token)
        dist.all_reduce(elapsed_time_s)
        global_tps = total_token / elapsed_time_s
        print(f"Global Tokens per Second: {global_tps.item():.2f}")
